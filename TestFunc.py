...
def TableFromImg(img):
    ...
    edged = cv2.Canny(tt1,10,50) 
    cv2.waitKey(0) 
  
    # Finding Contours 
    # Use a copy of the image e.g. edged.copy() 
    # since findContours alters the image 
    contours1, hierarchy = cv2.findContours(edged,cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) 
    ...
    for i in range(len(finallist)):
        for j in range(len(finallist[i])):
            to_out=''
            if(len(finallist[i][j])==0 or finallist[i][j] == []):
                pass
                #print('-')
                #todump.append(' ')
